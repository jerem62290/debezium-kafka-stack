# Stack CDC Kafka / Debezium hors-ligne sur RHEL 8.10 — v1.2.1, SELinux en Bash
...
- volumes montés et disponibles sous `/inte/vq/DAT/infr/KFK` et `/inte/vq/DAT/data/KFK` avant déploiement ; stockage Prometheus sous `/var/lib/prometheus` ;
- montage exécutable pour les binaires Kafka, système de fichiers permettant les labels SELinux (par exemple XFS/ext4) ;
- RPM `policycoreutils`, `policycoreutils-python-utils` et `libselinux-utils` disponibles dans les dépôts RHEL internes.
...
## Implantation Kafka de cette version

Les variables sont centralisées dans `inventories/production/group_vars/all.yml` et déjà renseignées :

| Variable | Valeur |
|---|---|
| `kafka_install_dir` | `/inte/vq/DAT/infr/KFK/kafka` |
| `kafka_data_dir` | `/inte/vq/DAT/data/KFK/kafka` |
| `kafka_metadata_dir` | `/inte/vq/DAT/data/KFK/kraft-metadata` |
| `kafka_config_dir` | `/inte/vq/DAT/data/KFK/kraft` |
| `kafka_service_user` | `ivqdikfk` |
| `kafka_service_group` | `ivqdkfk1` |

Le chemin de configuration a été normalisé avec le `/` initial. Les journaux applicatifs restent dans `/var/log/kafka`. Ces paramètres concernent les brokers ; Connect et la toolbox conservent leurs propres comptes et chemins.

Les comptes existants sont réutilisés via NSS/SSSD sans changement de leur UID, GID, home ou shell ; les comptes absents sont créés localement. Le groupe du processus Kafka est explicitement `ivqdkfk1`. Les données appartiennent au compte Kafka ; les binaires restent sous contrôle root et les configurations sont lisibles par le groupe de service.

SELinux reste enforcing : les types persistants sont appliqués avec `semanage fcontext`, puis `restorecon`, avant le formatage ou le démarrage. Les dépendances de montage et les droits Unix sont contrôlés. Voir [docs/kafka-layout-selinux.md](docs/kafka-layout-selinux.md) pour les labels, les limites et la migration d'une installation existante.

## 3. Déployer

Pour une **installation neuve**, après préparation des volumes, depuis la racine du projet :
...
Pour déplacer un Kafka déjà installé, suivez d'abord [la procédure de changement de chemins](docs/kafka-layout-selinux.md). Le playbook ne copie pas les anciens journaux Kafka vers les nouveaux répertoires.
...
| `kafka_layout_guard` | Vérifie les chemins, le changement de compte et la présence des métadonnées avant migration |
...
| `kafka` | Déploie 3 nœuds KRaft avec les chemins /inte, le compte dédié, les labels SELinux, SASL/PLAIN et JMX |
...
ansible-playbook tests/layout-guard-smoke.yml
bash tests/test-selinux-rules.sh
