...
def main():
    ...
    assert broker['log.dirs'] == '/inte/vq/DAT/data/KFK/kafka'
    assert broker['metadata.log.dir'] == '/inte/vq/DAT/data/KFK/kraft-metadata'
    kafka_unit = (root / 'kafka/kafka.service').read_text()
    assert 'User=ivqdikfk\n' in kafka_unit and 'Group=ivqdkfk1\n' in kafka_unit
    assert 'ExecStart=/inte/vq/DAT/infr/KFK/kafka/bin/kafka-server-start.sh /inte/vq/DAT/data/KFK/kraft/server.properties' in kafka_unit
    assert 'RequiresMountsFor=/inte/vq/DAT/infr/KFK/kafka /inte/vq/DAT/data/KFK/kraft ' in kafka_unit
    assert 'ReadOnlyPaths=/inte/vq/DAT/infr/KFK/kafka /inte/vq/DAT/data/KFK/kraft' in kafka_unit
    acl_script = (root / 'kafka/bootstrap-kafka-acls.sh').read_text()
    assert 'config="/inte/vq/DAT/data/KFK/kraft/broker-client.properties"' in acl_script
    assert '/etc/kafka/kraft' not in acl_script
