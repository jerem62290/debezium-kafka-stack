...
```bash
sudo -u ivqdikfk -g ivqdkfk1 /inte/vq/DAT/infr/KFK/kafka/bin/kafka-metadata-quorum.sh \
  --bootstrap-server kafka01.mondomaine:9092 \
  --command-config /inte/vq/DAT/data/KFK/kraft/broker-client.properties \
  describe --status
```
...
## Chemins /inte et SELinux

Les procédures de montage, droits, étiquetage et migration se trouvent dans [kafka-layout-selinux.md](kafka-layout-selinux.md). Le service systemd s'appelle toujours `kafka.service` ; son compte est `ivqdikfk:ivqdkfk1`. Les identités SASL restent les noms des brokers et ne sont pas renommées en fonction du compte Linux.
