# Passage de la version TLS à la variante sans PKI
...
- Le bundle, les versions des composants et le cluster ID restent identiques.
- **Depuis v1.2.0**, les répertoires Kafka sont sous `/inte` et le compte est `ivqdikfk:ivqdkfk1` : préparer également le transfert à froid décrit dans [kafka-layout-selinux.md](kafka-layout-selinux.md) avant de relancer.
